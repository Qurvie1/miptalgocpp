...
class BinarySearchTree {
 private:
  ...
  T* ParentsTraversal(NodeBST<T>* current, T* ptr) {
    if (current) {
      ptr = ParentsTraversal(current->left, ptr);
      if (current->left && current->right) {
        *(ptr++) = current->value;
      }
      ptr = ParentsTraversal(current->right, ptr);
    }
    return ptr;
  }
 ...
 public:
  ...
  void InOrderParentsTraversal(T* sequence) {
    ParentsTraversal(root_, sequence);
  }
...
int main() {
  ...
  bst.InOrderParentsTraversal(array);
  int i = 0;
  while (array[i] != 0) {
    std::cout << array[i++] << '\n';
  }
